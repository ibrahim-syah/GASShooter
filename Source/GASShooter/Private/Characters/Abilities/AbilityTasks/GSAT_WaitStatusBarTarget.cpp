...
#include "Characters/Abilities/AbilityTasks/GSAT_WaitStatusBarTarget.h"
		...
		if (!HitActor || HitActor != Ability->GetCurrentActorInfo()->AvatarActor.Get())
		{
			...
			if (bLookForStatusTargetActor && HitActor)
			{
					...
					== ECollisionResponse::ECR_Overlap)
				{
					...
					if (bIsStatusTarget && IGSDamageable::Execute_IsStatusBarAvailable(HitActor))
					{
						OutHitResult = Hit;
...
void UGSAT_WaitStatusBarTarget::PerformTrace()
{
	...
	// Default to end of trace line if we don't hit a valid, available StatusTarget Actor
	// bBlockingHit = valid, available StatusTarget Actor
	if (!ReturnHitResult.bBlockingHit)
	{
		// No valid, available StatusTarget Actor
