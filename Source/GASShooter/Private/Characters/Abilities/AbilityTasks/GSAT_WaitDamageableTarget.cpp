...
#include "Characters/Abilities/AbilityTasks/GSAT_WaitDamageableTarget.h"
...
UGSAT_WaitDamageableTarget::UGSAT_WaitDamageableTarget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bTraceAffectsAimPitch = true;
}

UGSAT_WaitDamageableTarget* UGSAT_WaitDamageableTarget::WaitForDamageableTarget(UGameplayAbility* OwningAbility, FName TaskInstanceName, FCollisionProfileName TraceProfile, float MaxRange, float TimerPeriod, bool bShowDebug)
{
	UGSAT_WaitDamageableTarget* MyObj = NewAbilityTask<UGSAT_WaitDamageableTarget>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
	...
	MyObj->StartLocation3P.SourceSocketName = "HeadInteractSource";
	MyObj->StartLocation3P.SourceAbility = OwningAbility;

	return MyObj;
}

void UGSAT_WaitDamageableTarget::Activate()
{
	UWorld* World = GetWorld();
	World->GetTimerManager().SetTimer(TraceTimerHandle, this, &UGSAT_WaitDamageableTarget::PerformTrace, TimerPeriod, true);
}

void UGSAT_WaitDamageableTarget::OnDestroy(bool AbilityEnded)
{
	UWorld* World = GetWorld();
...
void UGSAT_WaitDamageableTarget::LineTrace(FHitResult& OutHitResult, const UWorld* World, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params, bool bLookForDamageableActor) const
{
	check(World);
	...
	for (int32 HitIdx = 0; HitIdx < HitResults.Num(); ++HitIdx)
	{
		...
		{
			// If bLookForDamageableActor is false, we're looking for an endpoint to trace to
			if (bLookForDamageableActor && HitActor)
			{
				// bLookForDamageableActor is true, hit component must overlap COLLISION_INTERACTABLE trace channel
					...
					== ECollisionResponse::ECR_Overlap)
				{
					...
					bool bIsDamageable = HitActor->Implements<UGSDamageable>();

					if (bIsDamageable && IGSDamageable::Execute_IsAvailableForTakedown(HitActor, Hit.Component.Get()))
					{
						OutHitResult = Hit;
...
void UGSAT_WaitDamageableTarget::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params, const FVector& TraceStart, FVector& OutTraceEnd, bool bIgnorePitch) const
{
...
bool UGSAT_WaitDamageableTarget::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter, float AbilityRange, FVector& ClippedPosition) const
{
...
void UGSAT_WaitDamageableTarget::PerformTrace()
{
	...
	// Default to end of trace line if we don't hit a valid, available Damageable Actor
	// bBlockingHit = valid, available Damageable Actor
	if (!ReturnHitResult.bBlockingHit)
	{
		// No valid, available Damageable Actor

		ReturnHitResult.Location = TraceEnd;
		if (TargetData.Num() > 0 && TargetData.Get(0)->GetHitResult()->GetActor())
		{
			// Previous trace had a valid Damageable Actor, now we don't have one
			// Broadcast last valid target
			LostDamageableTarget.Broadcast(TargetData);
		}
	...
	{
		// Valid, available Damageable Actor
		...
		if (bBroadcastNewTarget)
		{
			...
			TargetData = MakeTargetData(ReturnHitResult);
			FoundNewDamageableTarget.Broadcast(TargetData);
		}
	}
...
FGameplayAbilityTargetDataHandle UGSAT_WaitDamageableTarget::MakeTargetData(const FHitResult& HitResult) const
{
